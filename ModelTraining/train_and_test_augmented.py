...
from tensorflow.keras.layers import GlobalAveragePooling2D, Dense, Dropout, RandomFlip, RandomRotation, RandomContrast, RandomBrightness
...
def train_chosen_model(model_name, dataset, data_type, id, config):
    ...
    model.compile(optimizer=keras.optimizers.AdamW(learning_rate=config['lr'], weight_decay=1e-4), 
    ...
    if config['ft_epochs'] != 0:
        ...
        model.compile(optimizer=keras.optimizers.AdamW(learning_rate=config['ft_lr'], weight_decay=1e-4), 
                    loss=keras.losses.CategoricalCrossentropy(label_smoothing=config['label_smoothing']),
                    metrics=metrics)
